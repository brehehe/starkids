...
class AdminReportActionIndex extends Component
{
    ...
    public function mount()
    {
        ...
        $this->products = \App\Models\Product\Product::whereIn('id', TransactionDetail::whereNotNull('product_id')
    ...
    public function render()
    {
        ...
        $topQty = (clone $baseQuery)
            ->select('product_id', 'name', \Illuminate\Support\Facades\DB::raw('SUM(quantity) as total_quantity'))
        ...
        $topRev = (clone $baseQuery)
            ->select('product_id', 'name', \Illuminate\Support\Facades\DB::raw('SUM(sub_total_price) as total_revenue'))
            ->groupBy('product_id', 'name')
