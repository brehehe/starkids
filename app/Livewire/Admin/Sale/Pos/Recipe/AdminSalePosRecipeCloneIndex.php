...
class AdminSalePosRecipeCloneIndex extends Component
{
        ...
        foreach ($transactionDetails as $key => $transactionDetail) {
            ...
            foreach ($transactionDetail->transactionDetail as $detail) {
                ...
                $this->transaction_details[$key]['details'][] = [
                    ...
                    'discount' => intval($detail->discount ?? 0),
                    'discount_input' => intval($detail->discount ?? 0),
                    'discount_type' => 'nominal',
                ...
                if (isset($this->transaction_details[$parentIndex]['details'][$childIndex])) {
                    ...
                    if ($transactionDetail) {
                        if ($field === 'quantity' || $field === 'quantity_real') {
                            ...
                            $currentDiscount = floatval($transactionDetail->discount ?? 0);
                            $transactionDetail->sub_total_price = max(0, ($currentPrice * $currentQty) - $currentDiscount);
                            ...
                            if ($detail['discount_type'] === 'percentage') {
                                ...
                                $transactionDetail->discount = round((($transactionDetail->price * $transactionDetail->quantity) * $percentage) / 100);
                            } else {
                                ...
                                if ($transactionDetail->discount > $maxDiscountAllowed) {
                        ...
                        $transactionDetail->sub_total_price = ($currentPrice * $transactionDetail->quantity) - $currentDiscount;
                        $transactionDetail->save();
                        ...
                        if ($inputType === 'percentage') {
                            $rowTotalBeforeDiscount = $transactionDetail->price * $transactionDetail->quantity;
                            $this->transaction_details[$parentIndex]['details'][$childIndex]['discount_input'] = ($rowTotalBeforeDiscount > 0 && $transactionDetail->discount > 0) ? (floatval($transactionDetail->discount) >= floatval($rowTotalBeforeDiscount) ? 100 : round(($transactionDetail->discount / $rowTotalBeforeDiscount) * 100, 2)) : 0;
                        } else {
        ...
        foreach ($this->transaction_details as $key => $value) {
            ...
            if (! empty($value['details'])) {
                foreach ($value['details'] as $detail) {
                    ...
                    // Keep the discount if nominal, or recalculate if it was percentage
                        ...
                        if ($medicineType->is_single) {
                            ...
                            if (! $productRecipe->is_non_stock) {
                                ...
                                if ($detail->quantity > $productStockRecipe->quantity) {
                                    // Hanya clamp quantity (yang akan diambil), JANGAN timpa quantity_real
                                    // (permintaan asli dokter) agar data historis tetap terjaga.
                                    $detail->quantity = $productStockRecipe->quantity;
                                    AlertHelper::error(
                                        'Gagal',
                                        "Stok produk '".($productRecipe?->name ?? 'Unknown').
                                            "' tidak mencukupi. Tersedia: {$productStockRecipe->quantity}, Diminta: {$detail->quantity_real}."
                                    );
                                }
                                ...
                                $detail->fill([
                                    ...
                                    'price' => $priceRecipe,
                                    'sub_total_price' => (isset($detail->discount) && $detail->discount > 0) ? ($priceRecipe * intval($detail->quantity_real)) - $detail->discount : $priceRecipe * intval($detail->quantity_real),
                                ])->save();
                            } else {
                                $detail->fill([
                                    'type' => $detail->type ?? 'single',
                ...
                if ($transactionDetail) {
                    if ($field === 'price') {
                        ...
                        if ($action['discount_type'] === 'percentage') {
                            ...
                            $percentage = $percentage > 100 ? 100 : ($percentage < 0 ? 0 : $percentage);
                            $transactionDetail->discount = round(($transactionDetail->price * $percentage) / 100);
                        } else {
                    ...
                    if ($inputType === 'percentage') {
                        $rowTotalBeforeDiscount = $transactionDetail->price * $transactionDetail->quantity;
                        $this->actions[$index]['discount_input'] = ($rowTotalBeforeDiscount > 0 && $transactionDetail->discount > 0) ? (floatval($transactionDetail->discount) >= floatval($rowTotalBeforeDiscount) ? 100 : round(($transactionDetail->discount / $rowTotalBeforeDiscount) * 100, 2)) : 0;
                    } else {
        ...
        if (count($parts) == 2) {
            ...
            if (isset($this->medicines[$index])) {
                ...
                if ($transactionDetail) {
                    ...
                    if ($inputType === 'percentage') {
                        $rowTotalBeforeDiscount = $transactionDetail->price * $transactionDetail->quantity;
                        $this->medicines[$index]['discount_input'] = ($rowTotalBeforeDiscount > 0 && $transactionDetail->discount > 0) ? (floatval($transactionDetail->discount) >= floatval($rowTotalBeforeDiscount) ? 100 : round(($transactionDetail->discount / $rowTotalBeforeDiscount) * 100, 2)) : 0;
                    } else {
                        $this->medicines[$index]['discount_input'] = intval($transactionDetail->discount);
                    }
    ...
    private function saveTransactionInstallments($transaction)
    {
        ...
        foreach ($breakdown as $item) {
            TransactionInstallment::create([
                'transaction_id' => $transaction->id,
                'tenor' => $item['tenor'],
                'due_date' => Carbon::createFromFormat('d/m/Y', $item['date'])->format('Y-m-d'),
                'amount' => $item['amount'],
                'status' => 'unpaid',
            ]);
        }
