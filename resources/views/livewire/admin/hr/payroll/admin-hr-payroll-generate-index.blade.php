        ...
        <div class="table-container">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="center">{{ $payrolls->firstItem() + $index }}</td>
                        <td>
                        ...
                        <td class="center">
                            @if($item->status == 'paid')
                                ...
                                <div class="text-[10px] text-gray-400 mt-1">{{ Carbon\Carbon::parse($item->payment_date)->format('d/m/Y') }}</div>
                        ...
                        <td class="center whitespace-nowrap">
                            <button wire:click="viewDetails('{{ $item->id }}')" class="btn-action btn-edit bg-blue-50 text-blue-600 hover:bg-blue-100 mr-2" title="Detail & Rincian">
                                <i class="fas fa-file-invoice-dollar"></i>
                            </button>
                            @if($item->status != 'paid')
                                <button wire:click="markAsPaid('{{ $item->id }}')" onclick="confirm('Tandai gaji {{ $item->user->name }} sebagai lunas dibayar?') || event.stopImmediatePropagation()" class="btn-action btn-edit bg-green-50 text-green-600 hover:bg-green-100" title="Bayar Sekarang">
                    ...
                    <tr>
                        <td colspan="8" class="no-data py-12">
                            <div class="flex flex-col items-center justify-center text-gray-500">
                                ...
                                <p class="text-sm text-gray-400">Belum ada *generate* gaji untuk periode <b>{{ Carbon\Carbon::parse($filterPeriod)->translatedFormat('F Y') }}</b>.</p>
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $payrolls->firstItem() ?? 0 }}</span> sampai <span
                        class="font-medium">{{ $payrolls->lastItem() ?? 0 }}</span> dari <span
                        class="font-medium">{{ $payrolls->total() ?? 0 }}</span> data rekap
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        {{ $payrolls->links('vendor.livewire.custom') }}
                    </nav>
                </div>
            </div>
        </div>
    ...
    <div wire:ignore.self id="modal-details" class="fixed inset-0 bg-overlay flex items-center justify-center z-50">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl transform transition-all scale-100 duration-300 ease-out animate-fade-in flex flex-col" style="max-height: 90vh;">
            ...
            <div class="flex justify-between items-center px-6 py-4 border-b bg-gray-50 rounded-t-2xl">
                <div>
                    <h2 class="text-xl font-bold text-[#1E3A8A] flex items-center">
                        ...
                        Slip Gaji ({{ Carbon\Carbon::parse($selectedPayroll->period)->translatedFormat('F Y') }})
                    </h2>
                    <p class="text-sm text-gray-500 font-medium mt-1">Pegawai: <span class="text-gray-800">{{ $selectedPayroll->user->name ?? '-' }}</span></p>
                </div>
            ...
            <div id="slip-content" class="p-8 overflow-y-auto bg-white">
                ...
                <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
                    <div>
                        ...
                        <div class="text-xl font-bold text-gray-800">Rp {{ number_format($selectedPayroll->basic_salary, 0, ',', '.') }}</div>
                    </div>
                    <div class="text-right">
                        ...
                        <div>
                            @if($selectedPayroll->status == 'paid')
                                ...
                                <div class="text-xs text-gray-500 mt-1">{{ Carbon\Carbon::parse($selectedPayroll->payment_date)->translatedFormat('d M Y') }}</div>
                            @elseif($selectedPayroll->status == 'published')
                                <span class="px-2 py-1 bg-yellow-100 text-yellow-700 rounded text-xs font-bold border border-yellow-200">TERBIT</span>
                            @else
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <ul class="space-y-3">
                            @forelse($selectedPayroll->details->where('type', 'allowance') as $detail)
                                <li class="flex justify-between items-center text-sm">
                                    <span class="text-gray-600">{{ $detail->name }}</span>
                                    <span class="font-medium text-gray-900 flex whitespace-nowrap">Rp <span class="ml-auto min-w-[70px] text-right">{{ number_format($detail->amount, 0, ',', '.') }}</span></span>
                                </li>
                            @empty
                            ...
                            <li class="flex justify-between items-center text-sm font-bold pt-3 mt-3 border-t border-gray-200">
                                ...
                                <span class="text-green-700 flex whitespace-nowrap">Rp <span class="ml-auto min-w-[70px] text-right">{{ number_format($selectedPayroll->total_allowance, 0, ',', '.') }}</span></span>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-3">
                            @forelse($selectedPayroll->details->where('type', 'deduction') as $detail)
                                <li class="flex justify-between items-center text-sm">
                                    <span class="text-gray-600">{{ $detail->name }}</span>
                                    <span class="font-medium text-gray-900 flex whitespace-nowrap">Rp <span class="ml-auto min-w-[70px] text-right">{{ number_format($detail->amount, 0, ',', '.') }}</span></span>
                                </li>
                            ...
                            <li class="flex justify-between items-center text-sm font-bold pt-3 mt-3 border-t border-gray-200">
                                ...
                                <span class="text-red-600 flex whitespace-nowrap">Rp <span class="ml-auto min-w-[70px] text-right">{{ number_format($selectedPayroll->total_deduction, 0, ',', '.') }}</span></span>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="mt-8 bg-[#1E3A8A] text-white rounded-xl p-5 shadow-lg flex items-center justify-between">
                    ...
                    <div class="text-3xl font-extrabold tracking-tight">
                        Rp {{ number_format($selectedPayroll->net_salary, 0, ',', '.') }}
                    </div>
                </div>
            ...
            <div class="p-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3 shrink-0 rounded-b-2xl">
                <button wire:click="downloadPDF('{{ $selectedPayroll->id }}')" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-sm transition-all focus:outline-none cursor-pointer flex items-center">
