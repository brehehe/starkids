        ...
        <div class="table-container">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="center">{{ $adjustments->firstItem() + $index }}</td>
                        <td>
                            <div class="font-medium text-gray-900">{{ $item->user->name ?? '-' }}</div>
                            <div class="text-xs text-gray-500">{{ $item->user->email ?? $item->user->phone ?? '-' }}</div>
                        </td>
                        <td>
                        ...
                        <td class="center whitespace-nowrap">
                            <button wire:click="edit('{{ $item->id }}')" class="btn-action btn-edit bg-blue-50 text-blue-600 hover:bg-blue-100 mr-2" title="Edit">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button wire:click="deleteConfirm('{{ $item->id }}')" class="btn-action btn-delete bg-red-50 text-red-600 hover:bg-red-100" title="Hapus">
                    ...
                    <tr>
                        <td colspan="6" class="no-data py-8">
                            <div class="flex flex-col items-center justify-center text-gray-500">
                                ...
                                <p>Belum ada data Potongan/Tambahan untuk periode <b>{{ Carbon\Carbon::parse($filterPeriod)->translatedFormat('F Y') }}</b>.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $adjustments->firstItem() ?? 0 }}</span> sampai <span
                        class="font-medium">{{ $adjustments->lastItem() ?? 0 }}</span> dari <span
                        class="font-medium">{{ $adjustments->total() ?? 0 }}</span> data
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        {{ $adjustments->links('vendor.livewire.custom') }}
                    </nav>
                </div>
            </div>
        </div>
    ...
    <div wire:ignore.self id="modal-adjustment"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all scale-95 duration-300 ease-out animate-fade-in">
            ...
            <div class="flex justify-between items-center p-6 border-b bg-gray-50 rounded-t-2xl">
                <h2 class="text-xl font-bold text-gray-800">{{ $data_id ? 'Edit Data' : 'Tambah Baru' }} ({{ Carbon\Carbon::parse($filterPeriod)->translatedFormat('F Y') }})</h2>
