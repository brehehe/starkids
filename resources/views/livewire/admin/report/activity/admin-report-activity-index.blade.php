...
<div id="report-container" class="w-full px-4 py-4">
    ...
    <ul class="flex flex-wrap text-sm font-medium text-center">
        @foreach($months as $key => $label)
            <li class="me-2 mb-2">
                <button wire:click="selectMonth('{{ $key }}')"
                    class="inline-block px-4 py-2.5 rounded-lg transition-all {{ $selectedMonth === $key ? 'text-white bg-blue-600 font-semibold' : 'text-gray-700 bg-gray-100 hover:text-blue-600 hover:bg-gray-200' }}">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm border-collapse">
                ...
                <tbody>
                    ...
                    @endphp
                    @foreach($dataUmum as $uraian => $data)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-center border border-gray-300 font-semibold">{{ $loop->iteration }}</td>
                            <td class="px-4 py-3 border border-gray-300">{{ $uraian }}</td>
                            <td class="px-4 py-3 border border-gray-300 {{ $loop->last ? 'font-semibold text-blue-600' : '' }}">{{ $data }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="p-4 bg-gray-100">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-xs border-collapse">
                    ...
                    <tbody>
                        @if(isset($activities['pain_data']['pain_data']) && $activities['pain_data']['pain_data']->count() > 0)
                            @foreach($activities['pain_data']['pain_data'] as $index => $pain)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-3 py-2 text-center border border-gray-300">{{ $index + 1 }}</td>
                                    <td class="px-3 py-2 border border-gray-300">{{ $pain['icd10_name'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['icd10_code'] }}</td>

                                    {{-- Age groups --}}
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['0-7_days']['male'] + $pain['age_groups']['0-7_days']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['8-28_days']['male'] + $pain['age_groups']['8-28_days']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['1-11_months']['male'] + $pain['age_groups']['1-11_months']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['1-4_years']['male'] + $pain['age_groups']['1-4_years']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['5-9_years']['male'] + $pain['age_groups']['5-9_years']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['10-14_years']['male'] + $pain['age_groups']['10-14_years']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['15-19_years']['male'] + $pain['age_groups']['15-19_years']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['20-44_years']['male'] + $pain['age_groups']['20-44_years']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['45-59_years']['male'] + $pain['age_groups']['45-59_years']['female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $pain['age_groups']['above_59_years']['male'] + $pain['age_groups']['above_59_years']['female'] }}</td>

                                    {{-- Gender totals --}}
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-blue-50 font-semibold">{{ $pain['total_male'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-pink-50 font-semibold">{{ $pain['total_female'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-200 font-bold">{{ $pain['grand_total'] }}</td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
                                <td colspan="16" class="px-3 py-4 text-center border border-gray-300 text-gray-500">
                                    Tidak ada data untuk bulan {{ $months[$selectedMonth] }} {{ $year }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
        ...
        <div class="p-4 bg-gray-50">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm border-collapse">
                    ...
                    <tbody>
                        @if(isset($activities['pain_data']['top_pain_data']) && $activities['pain_data']['top_pain_data']->count() > 0)
                            @foreach($activities['pain_data']['top_pain_data'] as $index => $disease)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-3 py-2 text-center border border-gray-300">{{ $index + 1 }}</td>
                                    <td class="px-3 py-2 border border-gray-300">{{ $disease['icd10_name'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-gray-50">{{ $disease['icd10_code'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-blue-50 font-semibold">{{ $disease['total_male'] }}</td>
                                    <td class="px-3 py-2 text-center border border-gray-300 bg-pink-50 font-semibold">{{ $disease['total_female'] }}</td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
        ...
        <div class="p-4 bg-gray-100">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm border-collapse">
                    ...
                    <tbody>
                        @if(isset($activities['clinic_visit_data']) && count($activities['clinic_visit_data']) > 0)
                        @foreach($activities['clinic_visit_data'] as $index => $row)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2 text-center border border-gray-300">{{ $index + 1 }}</td>
                                <td class="px-3 py-2 border border-gray-300">{{ $row['name'] }}</td>
                                <td class="px-3 py-2 text-center border border-gray-300 bg-blue-50">{{ $row['new_l'] }}</td>
                                <td class="px-3 py-2 text-center border border-gray-300 bg-pink-50">{{ $row['new_p'] }}</td>
                                <td class="px-3 py-2 text-center border border-gray-300 bg-blue-50">{{ $row['old_l'] }}</td>
                                <td class="px-3 py-2 text-center border border-gray-300 bg-pink-50">{{ $row['old_p'] }}</td>
                            </tr>
                        @endforeach
                        {{-- Total Row (Sum of Rawat Jalan & Rawat Inap only) --}}
                        <tr class="bg-gray-100 font-bold">
                            <td class="px-3 py-2 text-center border border-gray-300" colspan="2">Total</td>
                            <td class="px-3 py-2 text-center border border-gray-300 bg-blue-100">{{ collect($activities['clinic_visit_data'])->take(2)->sum('new_l') }}</td>
                            <td class="px-3 py-2 text-center border border-gray-300 bg-pink-100">{{ collect($activities['clinic_visit_data'])->take(2)->sum('new_p') }}</td>
                            <td class="px-3 py-2 text-center border border-gray-300 bg-blue-100">{{ collect($activities['clinic_visit_data'])->take(2)->sum('old_l') }}</td>
                            <td class="px-3 py-2 text-center border border-gray-300 bg-pink-100">{{ collect($activities['clinic_visit_data'])->take(2)->sum('old_p') }}</td>
                        </tr>
                    @else
                        <tr>
...
@push('scripts')
{{-- Load Standard XLSX first as fallback --}}
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
{{-- Load Styled XLSX (overwrites if successful) --}}
...
<script>
    async function exportExcelFull() {
        ...
        try {
            ...
            var allData = await @this.getAllMonthsExportData();
            ...
            if (hasData) {
                XLSX.writeFile(wb, "Laporan_Klinik_Full_" + "{{ $year }}" + ".xlsx");
            } else {
    ...
    function exportPdfFull() {
        ...
        var reportTitle = "Laporan_Klinik_{{ $selectedMonth }}_{{ $year }}";
