            ...
            <table class="table">
                ...
                <tbody>
                    @forelse ($deadStocks as $index => $deadStock)
                        <tr>
                            <td class="center">{{ $deadStocks->firstItem() + $index }}</td>
                            <td>{{ $deadStock->product->name ?? '-' }}</td>
                            <td>{{ $deadStock->code ?? '-' }}</td>
                            <td>{{ $deadStock->description ?? '-' }}</td>
                            <td>{{ $deadStock->quantity_old ?? '-' }}</td>
                            <td>{{ $deadStock->quantity ?? '-' }}</td>
                            <td>{{ number_format($deadStock->price, 0, ',', '.') ?? '-' }}</td>
                            <td>{{ number_format($deadStock->total, 0, ',', '.') ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $deadStocks->firstItem() }}</span> sampai <span
                        class="font-medium">{{ $deadStocks->lastItem() }}</span> dari <span
                        class="font-medium">{{ $deadStocks->total() }}</span> hasil
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        {{ $deadStocks->links('vendor.livewire.custom') }} <!-- Menampilkan pagination -->
                    </nav>
                </div>
            </div>
        </div>
