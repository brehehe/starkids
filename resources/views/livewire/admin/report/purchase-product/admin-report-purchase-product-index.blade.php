    ...
    <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-6">
        <div class="table-container">
            <table class="table">
                ...
                <tbody>
                    @forelse ($purchaseProducts as $index => $purchaseProduct)
                        <tr>
                            <td class="w-1 center">{{ $index + 1 }}</td>
                            <td>{{ $purchaseProduct->product->name }}</td>
                            <td class="center">{{ $purchaseProduct->quantity }}</td>
                            <td>
                                Rp{{ number_format($purchaseProduct->hna ?? 0, 0, ',', '.') }}
                            </td>
                            <td>
                                Rp{{ number_format($purchaseProduct->ppn ?? 0, 0, ',', '.') }}
                            </td>
                            <td>
                                Rp{{ number_format($purchaseProduct->sub_total ?? 0, 0, ',', '.') }}
                            </td>
                        </tr>
                    @empty
                        <tr>
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $purchaseProducts->firstItem() }}</span> sampai <span
                        class="font-medium">{{ $purchaseProducts->lastItem() }}</span> dari <span
                        class="font-medium">{{ $purchaseProducts->total() }}</span> hasil
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        {{ $purchaseProducts->links('vendor.livewire.custom') }} <!-- Menampilkan pagination -->
                    </nav>
                </div>
            </div>
        </div>
