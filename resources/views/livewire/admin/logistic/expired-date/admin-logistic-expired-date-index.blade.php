    ...
    <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-6">
        <div class="table-container">
            <table class="table">
                ...
                <tbody>
                    @forelse ($productExpiredDates as $index => $productExpiredDate)
                    @php
                    ...
                    @endphp
                    <tr class="{{ $rowClass }}">
                        <td class="center">{{ $productExpiredDates->firstItem() + $index }}</td>
                        <td>{{ $productExpiredDate?->product?->sku_number ?? '-' }}</td>
                        <td>
                            <div class="flex items-center gap-2">
                                {{ $productExpiredDate?->product?->name ?? '-' }}
                                @if(!$showOnlyNotified && $productExpiredDate->notification)
                        ...
                        <td>{{ $productExpiredDate?->expired_date ? \Carbon\Carbon::parse($productExpiredDate?->expired_date)->format('d F Y') : '-' }}</td>
                        <!-- <td>{{ $productExpiredDate?->batch_number }}</td> -->
                        <!-- <td>
                            <div class="flex items-center gap-1">
                                <span>@number($productExpiredDate?->quantity ?? 0)</span>
                                <span
                                    class="text-gray-500 text-sm">/{{ $productExpiredDate->product->unit->name ?? '-' }}</span>
                            </div>
                        </td> -->
                        @if($showOnlyNotified)
                            <td class="center">
                                @if($productExpiredDate->notification)
                                    <div class="flex flex-col items-center gap-1">
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                                            <i class="fas fa-clock mr-1"></i>
                                            {{ \Carbon\Carbon::parse($productExpiredDate->notification->data['expired_date'])->diffInDays() }} hari lagi
                                        </span>
                                        <span class="text-xs text-gray-500">
                                            {{ \Carbon\Carbon::parse($productExpiredDate->notification->created_at)->format('d/m/Y H:i') }}
                                        </span>
                                    </div>
                                @else
                        ...
                        <td class="center">
                            @if($showOnlyNotified)
                                @if($productExpiredDate->notification)
                                    <button wire:click="confirmDeleteNotification('{{ $productExpiredDate->id }}')"
                                ...
                                @endif
                            @else
                                <button wire:click="confirmDeleteProductExpiredDate('{{ $productExpiredDate->id }}')"
                ...
                @empty
                    <tr>
                        <td colspan="{{ $showOnlyNotified ? '8' : '7' }}" class="no-data">
                            @if($showOnlyNotified)
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $productExpiredDates->firstItem() }}</span> sampai <span
                        class="font-medium">{{ $productExpiredDates->lastItem() }}</span> dari <span
                        class="font-medium">{{ $productExpiredDates->total() }}</span> hasil
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        {{ $productExpiredDates->links('vendor.livewire.custom') }} <!-- Menampilkan pagination -->
                    </nav>
                </div>
            </div>
        </div>
