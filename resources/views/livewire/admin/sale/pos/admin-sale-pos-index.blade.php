<div>
    ...
    <div class="max-w-full mx-auto p-4 pt-16 grid grid-cols-1 lg:grid-cols-4 gap-6" style="margin-top: 50px;">
        <div class="bg-white rounded-xl shadow-md p-4 h-[calc(100vh-7rem)] flex flex-col md:col-span-4">
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="text-sm text-gray-600">
                    <div class="flex items-center gap-2">
                        ...
                        @endif
                        {{-- <select wire:model.live="select_printer_id"
                            class="px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            <option value="">Pilih Printer</option>
                            @foreach ($selectedPrinters as $printer)
                                <option value="{{ json_encode($printer) }}">
                                    [{{ strtoupper($printer['type']) }}] {{ $printer['name'] }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        {{-- <button onclick="choosePrinter()">🔍 Pilih Printer Bluetooth</button> --}}
            ...
            {{-- <div class="flex gap-4 mb-4">
                <div class="flex-1">
                    <select wire:model.live="patient_company_role_transaction_id"
                        class="w-full border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 p-2">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        @forelse ($transactions as $index => $transaction)
                            <tr wire:key="transaction-row-{{ $transaction->id }}-{{ $loop->index }}"
                                ...
                                <td class="py-3 px-4">Rp
                                    {{ number_format($transaction->grand_total_price ?? 0, 0, ',', '.') }}</td>
                                <td class="py-3 px-4">
                                    ...
                                    @switch($transaction->status)
                                        @case('completed')
                                            <button class="text-blue-600 hover:text-blue-800 mx-1"
                                                wire:click="openDetail('{{ $transaction->id }}')" title="Lihat Detail">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                            @if ($transaction->transaction_recipes_count > 0)
                                                <a target="_blank" href="{{ route('user.receipt.recipe', $transaction->id) }}"
                                                    class="text-yellow-600 hover:text-yellow-800 mx-1" title="Cetak">
                                                    <i class="fas fa-print"></i>
                                                </a>
                                            @endif
                                            <button onclick="printInvoiceFromPage('{{ $transaction->id }}','satuan')"
                                            ...
                                            <a target="_blank" href="{{ route('user.receipt.receipt', $transaction->id) }}"
                                                class="text-blue-600 hover:text-blue-800 mx-1" title="Cetak">
                                                <i class="fas fa-print"></i>
                                            </a>
                                            <button class="text-red-600 hover:text-red-800 mx-1"
                                                wire:click="confirmDelete('{{ $transaction->id }}')" title="Hapus">
                                        ...
                                        @case('process')
                                            @if (Auth::user()->hasRole(['Super Admin', 'Kasir']))
                                                <button class="text-blue-600 hover:text-blue-800 mx-1"
                                                    wire:click="openDetail('{{ $transaction->id }}')" title="Edit">
                                        ...
                                        @case('take_medicine')
                                            ...
                                            @if ($transaction->transaction_recipes_count > 0)
                                                <a target="_blank"
                                                    href="{{ route('user.receipt.recipe', $transaction->id) }}"
                                                    class="text-yellow-600 hover:text-yellow-800 mx-1" title="Cetak">
                                                    <i class="fas fa-print"></i>
                                                </a>
                                            @endif
    ...
    @push('scripts')
        <script>
            ...
            const CASH_DRAWER_ENABLED = @json($cashDrawer);
