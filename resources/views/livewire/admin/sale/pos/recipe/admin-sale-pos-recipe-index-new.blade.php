<div>
    ...
    <main class="max-w-full mx-auto p-4 pt-16 grid grid-cols-1 lg:grid-cols-4 gap-6" style="margin-top: 50px;">
                        ...
                        @foreach ($actions as $key_action => $action)
                            <tr class="border-b">
                                <td class="py-2" colspan="2">
                                    ...
                                    @if ($is_editable_price_pos && in_array($transaction->status,['draft','process']))
                                        <div class="flex gap-2 mt-2">
                                            <div class="flex items-center border rounded px-2 py-1 bg-white" title="Harga Awal">
                                                ...
                                                <input type="text" onkeyup="convertToRupiah(this)"
                                                    wire:model.lazy='actions.{{ $key_action }}.price'
                                            ...
                                            <div class="flex items-center border rounded bg-white overflow-hidden" title="Tipe & Nominal Diskon">
                                                <select wire:model='actions.{{ $key_action }}.discount_type' class="text-xs bg-gray-50 border-r py-1 px-1 focus:outline-none text-gray-600 appearance-none font-medium text-center cursor-pointer">
                                                    <option value="nominal">Rp</option>
                                                    <option value="percentage">%</option>
                                                </select>
                                                @if(($action['discount_type'] ?? 'nominal') == 'percentage')
                                                    <input type="text"
                                                        wire:model.lazy='actions.{{ $key_action }}.discount_input'
                                    ...
                                    @else
                                        <p class="text-xs text-gray-500">
                                            ...
                                            @if (isset($action['discount']) && $action['discount'] > 0)
                                                <span class="text-red-500 ml-1">(-Rp{{ number_format($action['discount'], 0, ',', '.') }})</span>
                                            @endif
                                        </p>
                                    @endif
                                </td>
                            ...
                            <tr class="border-t-4">
                                <td colspan="3" class="py-3 px-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <select
                                            {{ in_array($transaction->status, ['draft', 'process']) ? null : 'disabled' }}
                                            class="{{ in_array($transaction->status, ['draft', 'process']) ? null : 'bg-gray-100 cursor-not-allowed' }} text-sm border rounded px-2 py-1"
                                        ...
                                        <input type="text"
                                            ...
                                            class="{{ in_array($transaction->status, ['draft', 'process']) ? null : 'bg-gray-100 cursor-not-allowed' }} text-sm border rounded px-2 py-1 w-30">
                                    ...
                                    <div class="mt-2 text-sm text-gray-600">
                                        <input type="text"
                                            ...
                                            class="{{ in_array($transaction->status, ['draft', 'process']) ? null : 'bg-gray-100 cursor-not-allowed' }} w-full border rounded px-2 py-1">
                                    </div>
                                    <div class="mt-2 text-sm text-gray-600">
                                        <input type="text"
                                            {{ in_array($transaction->status, ['draft', 'process']) ? null : 'disabled' }}
                                ...
                                @foreach ($transaction_detail['details'] as $index => $item)
                                    <tr class="border-b">
                                        <td class="py-2" colspan="{{ !$transaction_detail['is_single'] ? 1 : 2 }}">
                                            ...
                                            @if ($is_editable_price_pos && in_array($transaction->status,['draft','process']) && (!isset($item['is_free_item']) || !$item['is_free_item']))
                                                <div class="flex gap-2 mt-2">
                                                    <div class="flex items-center border rounded px-2 py-1 bg-white" title="Harga Awal">
                                                        <span class="text-gray-500 mr-1 text-xs select-none">Rp</span>
                                                        <input type="text" onkeyup="convertToRupiah(this)"
                                                            wire:model.blur='transaction_details.{{ $key }}.details.{{ $index }}.price'
                                                            wire:blur="changedDetailField('{{ $key }}', '{{ $index }}', 'price')"
                                                            class="text-xs bg-transparent text-gray-700 focus:outline-none w-24" placeholder="Harga" />
                                                    </div>
                                                    <div class="flex items-center border rounded bg-white overflow-hidden" title="Tipe & Nominal Diskon">
                                                        <select
                                                            wire:model.blur='transaction_details.{{ $key }}.details.{{ $index }}.discount_type'
                                                            wire:blur="changedDetailField('{{ $key }}', '{{ $index }}', 'discount_type')"
                                                            ...
                                                            <option value="percentage" {{ ($item['discount_type'] ?? 'nominal') == 'percentage' ? 'selected' : '' }}>%</option>
                                                        </select>
                                                        @if(($item['discount_type'] ?? 'nominal') == 'percentage')
                                                            <input type="text"
                                                                wire:model.blur='transaction_details.{{ $key }}.details.{{ $index }}.discount_input'
                                                                wire:blur="changedDetailField('{{ $key }}', '{{ $index }}', 'discount_input')"
                                                                class="text-xs bg-transparent text-red-500 focus:outline-none w-24 px-2 py-1" placeholder="0" />
                                                        @else
                                                            <input type="text" onkeyup="convertToRupiah(this)"
                                                                wire:model.blur='transaction_details.{{ $key }}.details.{{ $index }}.discount_input'
                                            ...
                                            @else
                                                <p class="text-xs text-gray-500 flex ">
                                                    @if (isset($item['price_discount']) && $item['price_discount'] != $item['price'])
                                                        <span class="line-through text-red-500 mr-2">
                                                            Rp{{ number_format($item['price_discount'], 0, ',', '.') }}
                                                        </span>
                                                    @endif
                                                    <span
                                                        class="{{ isset($item['is_free_item']) && $item['is_free_item'] ? 'text-green-600 font-medium' : '' }}">
                                                        @if (isset($item['is_free_item']) && $item['is_free_item'])
                                                            GRATIS
                                                            (Rp{{ number_format($item['price_discount'] ?? 0, 0, ',', '.') }})
                                                        @else
                                                            Rp{{ number_format($item['price'], 0, ',', '.') }}
                                                            @if (isset($item['discount']) && $item['discount'] > 0)
                                                                <span class="text-red-500 ml-1">(-Rp{{ number_format($item['discount'], 0, ',', '.') }})</span>
                                                            @endif
                                                        @endif
                                                    </span>
                                                </p>
                                        ...
                                        @if (!$transaction_detail['is_single'])
                                            <td class="py-2">
                                                <input
                                                    wire:model.blur="transaction_details.{{ $key }}.details.{{ $index }}.quantity"
                                                    wire:blur="changedDetailField('{{ $key }}', '{{ $index }}', 'quantity')"
                                                    type="text" placeholder="Quantity Permintaan"
                                                    {{ in_array($transaction->status, ['draft', 'process']) ? null : 'disabled' }}
                                                    class="{{ in_array($transaction->status, ['draft', 'process']) ? null : 'bg-gray-100 cursor-not-allowed' }} text-sm border rounded px-2 py-1 w-48">
                                            </td>
                                        @endif
                                        <td class="py-2 text-center">
                                            {{ $item['quantity'] }}
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            ...
                                            <div
                                                ...
                                                @if ($is_editable_price_pos && in_array($transaction->status,['draft','process']) && (!isset($medicine['is_free_item']) || !$medicine['is_free_item']))
                                                    <div class="flex gap-2 mt-2">
                                                        <div class="flex items-center border rounded px-2 py-1 bg-white" title="Harga Awal">
                                                            <span class="text-gray-500 mr-1 text-xs select-none">Rp</span>
                                                            <input type="text" onkeyup="convertToRupiah(this)"
                                                                wire:model.lazy='medicines.{{ $key_medicine }}.price'
                                                                class="text-xs bg-transparent text-gray-700 focus:outline-none w-24" placeholder="Harga" />
                                                        </div>
                                                        <div class="flex items-center border rounded bg-white overflow-hidden" title="Tipe & Nominal Diskon">
                                                            <select wire:model='medicines.{{ $key_medicine }}.discount_type' class="text-xs bg-gray-50 border-r py-1 px-1 focus:outline-none text-gray-600 appearance-none font-medium text-center cursor-pointer">
                                                                <option value="nominal">Rp</option>
                                                                <option value="percentage">%</option>
                                                            </select>
                                                            @if(($medicine['discount_type'] ?? 'nominal') == 'percentage')
                                                                <input type="text"
                                                                    wire:model.lazy='medicines.{{ $key_medicine }}.discount_input'
                                                                    class="text-xs bg-transparent text-red-500 focus:outline-none w-24 px-2 py-1" placeholder="0" />
                                                            @else
                                                                <input type="text" onkeyup="convertToRupiah(this)"
                                                                    wire:model.lazy='medicines.{{ $key_medicine }}.discount_input'
                                                    ...
                                                    <p class="text-xs text-gray-500 flex ">
                                                        ...
                                                        <span
                                                            ...
                                                            @else
                                                                ...
                                                                @if (isset($medicine['discount']) && $medicine['discount'] > 0)
                                                                    <span class="text-red-500 ml-1">(-Rp{{ number_format($medicine['discount'], 0, ',', '.') }})</span>
                                                                @endif
                                                            @endif
                                                        </span>
                                                    </p>
        ...
        <div class="bg-white rounded-xl shadow-md p-4 flex flex-col">
            ...
            <div class="flex-1 space-y-4">
                    ...
                    @if (!empty($this->installment_breakdown))
                        <div class="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-xs text-left">
                                    ...
                                    <tbody class="divide-y divide-blue-100">
                                        @foreach ($this->installment_breakdown as $item)
                                            <tr>
                                                <td class="py-1.5">{{ $item['tenor'] }}x</td>
                                                <td class="py-1.5">{{ $item['date'] }}</td>
                                                <td class="py-1.5 text-right font-medium">Rp {{ number_format($item['amount'], 0, ',', '.') }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            ...
                            <div class="mt-2 pt-2 border-t border-blue-200 flex justify-between items-center text-xs">
                                ...
                                <span class="font-bold text-blue-700">Rp {{ number_format(max(0, ($transaction->grand_total_price ?? 0) - ($transaction->payment_amount ?? 0)), 0, ',', '.') }}</span>
                            </div>
                        </div>
                    @endif
                @endif
                ...
                <div class="space-y-3 mt-3">
                    <div class="flex justify-between items-center">
                        <label class="block text-sm font-medium">Metode Pembayaran {{ $is_pending_payment ? '(Down Payment)' : '' }}</label>
                        @if (in_array($transaction->status, ['draft', 'process']))
            ...
            @elseif (in_array($transaction->status, ['process']))
