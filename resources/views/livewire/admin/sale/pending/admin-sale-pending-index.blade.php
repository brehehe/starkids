    ...
    <div class="mb-6">
        <div class="flex p-1 bg-gray-100 rounded-lg w-fit">
            <button wire:click="setStatus('draft')"
                class="px-4 py-2 text-sm font-medium rounded-md transition-all {{ $status_payment === 'draft' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                Draft
            </button>
            <button wire:click="setStatus('paid')"
                class="px-4 py-2 text-sm font-medium rounded-md transition-all {{ $status_payment === 'paid' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                Paid
            </button>
        </div>
    </div>
    ...
    <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-6">
                ...
                <tbody>
                    @forelse ($transactions as $index => $transaction)
                        <tr>
                            ...
                            <td>
                                <div class="font-medium text-gray-900">{{ $transaction->patient_name ?? '-' }}</div>
                                <div class="text-xs text-gray-500">{{ $transaction->patient?->member_id ?? '' }}</div>
                            </td>
                            <td>
                                @if ($transaction->installment_count)
                                    <div>
                                        <span class="font-medium text-blue-600">{{ $transaction->installment_count }}x</span>
                                        <span
                                            class="text-[10px] text-gray-400 capitalize">({{ $transaction->installment_period ?? '-' }})</span>
                                    </div>
                                @else
                            ...
                            <td>
                                @if ($transaction->installment_count)
                                    ...
                                    @endphp
                                    <span class="font-medium {{ $remainingTenor > 0 ? 'text-orange-600' : 'text-green-600' }}">
                                        {{ $remainingTenor }}x
                                    </span>
                                @else
                                    <span class="text-gray-300">-</span>
                            ...
                            <td class="text-right font-medium">Rp @number($transaction->grand_total_price)</td>
                            <td class="text-right font-medium text-green-600">Rp @number($transaction->payment_amount)</td>
                            <td class="text-right font-medium text-red-600">Rp @number($transaction->remaining_bill)</td>
                            <td class="center">
                                @if($transaction->status_payment == 'paid')
                                    <span class="badge badge-success">Paid</span>
                                @elseif($transaction->status_payment == 'draft')
                                    <span class="badge badge-gray">Draft</span>
                                @else
                                    <span class="badge badge-warning">{{ ucfirst($transaction->status_payment) }}</span>
                                @endif
                            </td>
                            <td class="center">
                                <div class="flex items-center gap-1 justify-center">
                                    <a href="{{ route('user.sale.pending.detail', ['transaction_id' => $transaction->id]) }}"
                                    ...
                                    <button onclick="printInvoiceFromPage('{{ $transaction->id }}','total')"
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                ...
                <div>
                    {{ $transactions->links('vendor.livewire.custom') }}
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    <script>
        ...
        const PAPER_WIDTH = {{ $paperWidth ?? 48 }}; // atur sesuai printer kamu

        // 🔑 flag dari backend (true / false)
        const CASH_DRAWER_ENABLED = @json($cashDrawer ?? false);
        ...
        async function printInvoiceFromPage(transactionId, type = 'satuan') {
            try {
                ...
                // --- Logo Section ---
                let storeName = doc.querySelector(".store-name")?.innerText.trim() ?? "{{config('app.name')}}";
