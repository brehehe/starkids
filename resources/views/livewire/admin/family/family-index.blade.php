            ...
            <div class="bg-white shadow-xl sm:rounded-lg p-6">
                ...
                <div class="mb-6 relative">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm relative">
                        ...
                        @if($search && !$family && empty($search_results) && !$foundUser)
                    ...
                    @if(!empty($search_results))
                        <ul class="absolute z-50 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                            @foreach($search_results as $result)
                                <li wire:click="selectPatient('{{ $result['id'] }}')" class="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-gray-100 border-b border-gray-100 last:border-0">
                                    <div class="flex flex-col">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span class="text-xs text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">
                                                {{ $result['company_roles'][0]['medical_record_number'] ?? 'No RM' }}
                                            </span>
                                        </div>
                                        <div class="text-xs text-gray-500 mt-0.5">
                                            NIK: {{ $result['user_detail']['identity_card'] ?? '-' }} | Email: {{ $result['email'] ?? '-' }}
                                        </div>
                                    </div>
                                </li>
                            @endforeach
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm leading-5 text-yellow-700">
                                    <p>
                                        Pasien <strong>{{ $foundUser->name }}</strong> belum memiliki data keluarga.
                                    </p>
                                    <p class="mt-2">
                                        <button wire:click="createFamilyForUser('{{ $foundUser->id }}')" class="btn btn-primary btn-sm">
                                            Buat Keluarga Baru untuk {{ $foundUser->name }}
                                        </button>
                                    </p>
                                </div>
                            </div>
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        <div class="flex justify-between items-center mb-4">
                            <div>
                                ...
                                <p class="mt-1 max-w-2xl text-sm text-gray-500">Kepala Keluarga: {{ $family->headUser->name ?? '-' }}</p>
                            </div>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <div class="text-sm text-gray-500">{{ $member->user->email ?? $member->user->phone }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $member->user_id === $family->head_user_id ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                {{ ucfirst($member->relationship) }}
                                                @if($member->user_id === $family->head_user_id) (Master) @endif
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $member->user->companyRoles->first()?->medical_record_number ?? '-' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            @if($member->user_id !== $family->head_user_id)
                                                <button wire:click="setHead('{{ $member->id }}')" class="text-indigo-600 hover:text-indigo-900 mr-3">Set Master</button>
                                                <button wire:click="removeMember('{{ $member->id }}')" class="text-red-600 hover:text-red-900">Hapus</button>
                                            @endif
                                        </td>
                                    </tr>
                                    @endforeach
                                    ...
                                    @if($selected_user)
                                        <div class="bg-indigo-50 border border-indigo-100 rounded-md p-3">
                                            <div class="flex justify-between items-start">
                                                <div>
                                                    <h4 class="text-sm font-bold text-indigo-900">{{ $selected_user->name }}</h4>
                                                    <div class="text-xs text-indigo-700 mt-1">
                                                        <p>Email: {{ $selected_user->email ?? '-' }}</p>
                                                        <p>No. RM: {{ $selected_user->companyRoles->first()?->medical_record_number ?? '-' }}</p>
                                                        <p>NIK: {{ $selected_user->userDetail->identity_card ?? '-' }}</p>
                                                    </div>
                                                </div>
                                            ...
                                            @if(!empty($user_results))
                                                <ul class="absolute z-50 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                                                    @foreach($user_results as $result)
                                                        <li wire:click="selectMember('{{ $result->id }}')" class="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-gray-100">
                                                            <div class="flex items-center">
                                                                <span class="font-normal block truncate">
                                                                    {{ $result->name }} ({{ $result->companyRoles->first()?->medical_record_number ?? 'No RM' }})
                                                                </span>
                                                            </div>
                                                        </li>
                                                    @endforeach
