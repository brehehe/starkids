        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-3xl font-bold mt-2">{{ number_format($this->totalTransactions, 0, ',', '.') }}</h3>
                </div>
            ...
            <div>
                ...
                <div wire:key="select-{{ rand() }}">
                    <select class="form-control" x-data x-ref="input" x-init="$($refs.input).selectize({
                        ...
                        @foreach ($accountOptions as $key_account => $account)
        ...
        <div class="space-y-4">
            @forelse ($accounts as $account)
                <div x-data="{ open: false }" class="bg-white border rounded-lg shadow-sm overflow-hidden">
                    {{-- Account Header / Accordion Trigger --}}
                    <div @click="open = !open" class="p-4 bg-gray-50 flex items-center justify-between cursor-pointer hover:bg-gray-100 transition-colors">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-800">{{ $account->code }} - {{ $account->name }}</h3>
                                <p class="text-sm text-gray-500">{{ $account->accountTransactions->count() }} Transaksi</p>
                            </div>
                        </div>
                        <div class="flex space-x-8">
                            <div class="text-right">
                                <p class="text-xs text-gray-500 uppercase">Total Debit</p>
                                <p class="font-bold text-green-600">Rp {{ number_format($account->accountTransactions->sum('debit'), 0, ',', '.') }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-gray-500 uppercase">Total Kredit</p>
                                <p class="font-bold text-red-600">Rp {{ number_format($account->accountTransactions->sum('credit'), 0, ',', '.') }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-gray-500 uppercase">Saldo</p>
                                <p class="font-bold text-blue-600">Rp {{ number_format($account->accountTransactions->sum('debit') - $account->accountTransactions->sum('credit'), 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>

                    {{-- Accordion Body / Transactions Table --}}
                    <div x-show="open" x-collapse class="border-t">
                        <div class="table-container">
                            <table class="table">
                                ...
                                <tbody>
                                    @foreach ($account->accountTransactions as $index => $transaction)
                                        <tr class="hover:bg-gray-50">
                                            <td class="center">{{ $index + 1 }}</td>
                                            <td>{{ $transaction->journalItem->code ?? '-' }}</td>
                                            <td>{{ $transaction->date ? \Carbon\Carbon::parse($transaction->date)->locale('id')->isoFormat('D MMMM Y') : '-' }}</td>
                                            <td>{{ $transaction->description ?? '-' }}</td>
                                            <td class="text-right">{{ $transaction->debit ? 'Rp ' . number_format($transaction->debit, 0, ',', '.') : '-' }}</td>
                                            <td class="text-right">{{ $transaction->credit ? 'Rp ' . number_format($transaction->credit, 0, ',', '.') : '-' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
        ...
        <div class="px-5 py-4 bg-gray-50/80 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan <span class="font-medium">{{ $accounts->firstItem() }}</span> sampai <span
                        class="font-medium">{{ $accounts->lastItem() }}</span> dari <span
                        class="font-medium">{{ $accounts->total() }}</span> hasil
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        {{ $accounts->links('vendor.livewire.custom') }} <!-- Menampilkan pagination -->
                    </nav>
                </div>
            </div>
        </div>
