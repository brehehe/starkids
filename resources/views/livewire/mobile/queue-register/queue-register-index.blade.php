<div>
    ...
    <div class="px-5 sm:px-6 py-6 space-y-5">
        ...
        <section class="space-y-2">
            ...
            <div class="space-y-4">
                @foreach($patients as $p)
                    @php $isActive = $selectedPatientId === $p['id']; @endphp

                    <button type="button"
                            wire:key="patient-{{ $p['id'] }}"
                            {{-- wire:click="selectPatient({{ $p['id'] }})" --}}
                            wire:click="selectPatient(@js($p['id']))"
                        ...
                        {{-- stroke aktif: pakai pseudo layer di DALAM (anti clip + anti focus bug) --}}
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-1 flex items-center justify-between gap-3">
                                ...
                                <div class="font-bold text-slate-900 text-sm">{{ $p['rm'] }}</div>
                            </div>

                            <div class="mt-1">
                                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-light {{ $p['tagClass'] }}">
                                    {{ $p['tag'] }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="pt-6">
                            <div class="w-6 h-6 rounded-lg flex items-center justify-center
                                        {{ $isActive ? 'bg-sky-500 text-white' : 'bg-white border-2 border-slate-200 text-transparent' }}">
            ...
            {{-- tambah pasien --}}
            <a href="{{ route('mobile.queue.register.create-patient') }}" type="button" class="w-full h-10 rounded-full bg-sky-400 text-white font-extrabold text-md mt-3 flex items-center justify-center gap-2">
        ...
        <section class="space-y-2.5">
                    ...
                    @foreach($dates as $d)
                        ...
                        @if($monthChanged)
                            ...
                            <div class="ml-1 mr-1 flex items-center gap-2">
                                <span class="text-xs font-extrabold text-slate-600">{{ $d['month'] }}</span>
                                <span class="h-6 w-px bg-slate-200"></span>
                            </div>
                            @php $lastMonth = $d['month']; @endphp
                        @endif

                        <button type="button"
                                wire:key="date-{{ $d['key'] }}"
                                wire:click="selectDate('{{ $d['key'] }}')"
                                class="w-16 h-16 rounded-2xl flex flex-col items-center justify-center
                                    font-extrabold shadow-sm transition
                                    [-webkit-tap-highlight-color:transparent]
                                    {{ $active ? 'bg-sky-400 text-white shadow-[0_12px_25px_rgba(14,165,233,0.25)]' : 'bg-slate-400/60 text-white/95' }}"
                        >
                            @if($d['is_today'])
                                <div class="text-[11px] leading-none">Hari</div>
                                <div class="text-[11px] leading-none mt-0.5">ini</div>
                            @else
                                <div class="text-sm">{{ $d['dow'] }}</div>
                                <div class="text-base leading-none">{{ $d['day'] }}</div>
                            @endif
                        </button>
                    @endforeach
                </div>
            ...
            {{-- kategori dokter (horizontal scroll) --}}
            <div class="grid grid-cols-2 gap-3">
                @foreach($polyclinics as $c)
                    @php $active = $selectedPolyclinic === $c['key']; @endphp

                    <button type="button"
                            wire:key="cat-{{ $c['key'] }}"
                            wire:click="selectPolyclinic(@js($c['key']))"
                        ...
                        {{-- BORDER LAYER (ini yang menggantikan border button) --}}
                        <span class="pointer-events-none absolute inset-0 rounded-2xl border-2
                                    {{ $active ? 'border-sky-500' : 'border-slate-200' }}"></span>
            ...
            <div class="space-y-2.5 pt-2">
                @foreach($doctorSchedules as $s)
                    @php $isSelected = $selectedScheduleId === $s['id']; @endphp

                    {{-- Outer wrapper buat drop shadow (biar inner bisa overflow-hidden tanpa ngilangin shadow luar) --}}
                    <div class="rounded-3xl shadow-[0_12px_30px_rgba(15,23,42,0.10)]">
                        <button type="button"
                                wire:key="schedule-{{ $s['id'] }}"
                                wire:click="selectSchedule(@js($s['id']))"
                                class="relative w-full text-left rounded-3xl bg-white p-3 flex gap-3 items-center transition
                                    overflow-hidden
                                    [-webkit-tap-highlight-color:transparent]
                                    focus:outline-none"
                        >
                            {{-- Stroke di dalam (INI yang menggantikan border/ring) --}}
                            <span class="pointer-events-none absolute inset-0 rounded-3xl
                                        {{ $isSelected
                                            ? 'shadow-[inset_0_0_0_2px_rgba(14,165,233,1)]'
                                            : 'shadow-[inset_0_0_0_2px_rgba(226,232,240,1)]'
                                        }}"></span>
                            ...
                            <div class="flex-1 min-w-0 relative">
                                ...
                                {{-- waktu jadi chip (jangan duplicate) --}}
        ...
        {{-- CTA: LIHAT ANTRIAN (fixed bottom) --}}
        <div class="fixed inset-x-0 bottom-[5.25rem] sm:bottom-[5.25rem] z-[50]">
            ...
            <div class="px-3 sm:px-4">
                <div class="mx-auto w-full max-w-[430px] sm:max-w-[640px] md:max-w-[800px] {{ !$buttonCreate ? 'hidden' : '' }}">
                    <button
                        type="button"
                        x-data
