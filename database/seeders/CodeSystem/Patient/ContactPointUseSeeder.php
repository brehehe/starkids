...
use App\Models\Master\CodeSystem\Patient\MasterPatientContactPointUse;
...
class ContactPointUseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        //
        // Ambil isi file JSON
        $json = File::get(database_path('seeders/CodeSystem/Files/Patient/CodeSystem-contact-point-use.json'));

        $datas = isset(json_decode($json, true)['concept']) ? json_decode($json, true)['concept'] : [];

        foreach ($datas as $key => $value) {

            MasterPatientContactPointUse::create([
