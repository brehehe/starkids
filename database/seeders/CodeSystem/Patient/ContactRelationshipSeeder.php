...
use App\Models\Master\CodeSystem\Patient\ContactRelationship;
use App\Models\Master\CodeSystem\Patient\MasterPatientContactRelationship;
use App\Models\Master\CodeSystem\Patient\MasterPatientContractRelationship;
...
class ContactRelationshipSeeder extends Seeder
{
    ...
    public function run(): void
    {
        //
        // Ambil isi file JSON
        $json = File::get(database_path('seeders/CodeSystem/Files/Patient/CodeSystem-contact-relationship.json'));

        $datas = isset(json_decode($json, true)['concept']) ? json_decode($json, true)['concept'] : [];

        foreach ($datas as $key => $value) {

        MasterPatientContactRelationship::create([
