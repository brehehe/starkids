...
class DosageDoseRateSeeder extends Seeder
{
    ...
    public function run(): void
    {
        //
        // Ambil isi file JSON
        $json = File::get(database_path('seeders/CodeSystem/Files/MedicationRequest/CodeSystem-medicationrequest-dosage-dose-rate.json'));

        $datas = isset(json_decode($json, true)['concept']) ? json_decode($json, true)['concept'] : [];

        foreach ($datas as $key => $value) {

        MasterMedicationRequestDosageDoseRate::create([
