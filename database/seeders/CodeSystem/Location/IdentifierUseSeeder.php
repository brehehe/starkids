...
use App\Models\Master\CodeSystem\Location\MasterLocationIdentifierUse;
...
class IdentifierUseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        //
        // Ambil isi file JSON
        $json = File::get(database_path('seeders/CodeSystem/Files/Location/CodeSystem-identifier-use.json'));

        $datas = isset(json_decode($json, true)['concept']) ? json_decode($json, true)['concept'] : [];

        foreach ($datas as $key => $value) {

        MasterLocationIdentifierUse::create([
